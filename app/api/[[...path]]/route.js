...
// Database module - loaded dynamically
let db = null;
...
const handler = async (request, context) => {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { 
          ...
          hint: 'Please ensure POSTGRES_URL is correctly set in .env file'
        },
    ...
    if (path === 'settings' && method === 'GET') {
      const result = await database.query('SELECT key, value FROM site_settings');
      const settings = {};
    ...
    // Get membership tiers with benefits (PUBLIC - FROM DATABASE)
    if (path === 'tiers' && method === 'GET') {
      ...
      for (const tier of tiersResult.rows) {
        const benefitsResult = await database.query(
    ...
    if (path.startsWith('pages/') && !path.includes('type/') && !path.startsWith('pages/admin') && method === 'GET') {
    ...
    if (path.startsWith('admin/settings/') && method === 'PUT') {
      ...
      const existing = await database.query('SELECT id FROM site_settings WHERE key = $1', [key]);
    ...
    if (path === 'admin/tiers' && method === 'GET') {
      const tiersResult = await database.query('SELECT * FROM membership_tiers ORDER BY sort_order ASC');
      const tiers = [];
      for (const tier of tiersResult.rows) {
        const benefitsResult = await database.query(
      ...
      const result = await database.query(
        `INSERT INTO membership_tiers 
         (name, subtitle, headline, description, multiplier, min_tokens, min_spend, min_visits, 
          instant_upgrade_amount, icon_url, bg_color, text_color, button_style, sort_order, is_active) 
        ...
        [body.name, body.subtitle, body.headline, body.description, body.multiplier, 
         ...
         body.icon_url, body.bg_color || '#FFFFFF', body.text_color || 'dark', 
         body.button_style || 'dark', body.sort_order || 0, body.is_active !== false]
      );
      return NextResponse.json(result.rows[0], { headers: corsHeaders });
    }
    ...
    if (path.startsWith('admin/tiers/') && !path.includes('/benefits') && method === 'DELETE') {
      const tierId = path.replace('admin/tiers/', '');
      await database.query('DELETE FROM membership_tiers WHERE id = $1', [tierId]);
      return NextResponse.json({ success: true }, { headers: corsHeaders });
    }
    ...
    if (path.startsWith('admin/benefits/') && method === 'PUT') {
      ...
      await database.query(
        ...
        [body.title, body.description, body.icon_name, body.sort_order, body.is_active !== false, benefitId]
      );
    ...
    if (path.startsWith('admin/benefits/') && method === 'DELETE') {
      const benefitId = path.replace('admin/benefits/', '');
      await database.query('DELETE FROM tier_benefits WHERE id = $1', [benefitId]);
      return NextResponse.json({ success: true }, { headers: corsHeaders });
    }

    // Admin: Get all categories
    if (path === 'admin/categories' && method === 'GET') {
      const result = await database.query('SELECT * FROM service_categories ORDER BY sort_order ASC');
      return NextResponse.json(result.rows, { headers: corsHeaders });
    }
    ...
    if (path.startsWith('admin/categories/') && method === 'DELETE') {
      const categoryId = path.replace('admin/categories/', '');
      await database.query('DELETE FROM service_categories WHERE id = $1', [categoryId]);
      return NextResponse.json({ success: true }, { headers: corsHeaders });
    }

    // Admin: Get all pages
    if (path === 'admin/pages' && method === 'GET') {
      const result = await database.query('SELECT * FROM pages ORDER BY created_at DESC');
      return NextResponse.json(result.rows, { headers: corsHeaders });
    }
    ...
    // Admin: Initialize/Reset database
